...
class Speed(Callback):
    ...
    def on_train_batch_end(
        ...
        run_training_batch_time = \
            trainer.profiler.recorded_durations['run_training_batch'][-1]
        if hasattr(trainer.datamodule, 'batch_size'):
            total_batch_size = trainer.datamodule.batch_size * trainer.world_size
            legacy_metrics['speed'] = \
                1. * total_batch_size / run_training_batch_time
        else:
    ...
    def on_train_epoch_end(self, trainer, pl_module, *args, **kwargs):
        ...
        legacy_metrics = \
            trainer.logger_connector.cached_results.legacy_batch_log_metrics
        run_training_epoch_time = \
            trainer.profiler.recorded_durations['run_training_epoch']
        if (len(run_training_epoch_time) > 0 and
